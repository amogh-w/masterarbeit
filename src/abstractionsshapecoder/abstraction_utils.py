...
def integrate_abstractions(
    ...
    # Use local definitions instead of importing from other modules
    ...
    # Try to import these from the package if they are not local, 
    # but for Abstraction and egglog functions we should use the ones in this file.
    try:
        from abstractionssymh.abstraction_compare_utils import get_point_cloud_from_dsl, calculate_chamfer_distance
    except ImportError:
        # Fallback if the package is not structured as expected
        pass
    ...
    if len(child_nodes) == 1:
        ...
        # --- Failsafe: only pair if current_node is not already an Abstraction ---
        if isinstance(current_node, Abstraction):
            return current_node
