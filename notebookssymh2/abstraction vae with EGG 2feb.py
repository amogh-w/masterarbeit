...
for key in tqdm(test_keys, desc="Processing"):
    try:
        # 1. Start ONLY from Raw Geometry
        orig_dsl = all_dsl_shapes[key]['dsl']

        egraph = EGraph()
        sym_map = SymbolicValueMap(snap_grid=0.02)
        e_root = egraph.let("root", to_egglog_final(orig_dsl, sym_map))

        # 2. Run the engine and capture the report
        # We increase iterations since we aren't giving it a 'greedy' starting point
        report = egraph.run(
            back_off().scope(run(get_integrated_ruleset(), scheduler=back_off()) * 15)
        )
        ...
        for i, iteration in enumerate(report.iterations):
            print(f"Iteration {i}:")
            # This shows which rules actually found matches and changed the graph
            for rule_name, rule_reports in iteration.rule_set_report.rule_reports.items():
                total_matches = sum(r.num_matches for r in rule_reports)
                if total_matches > 0:
                    # We clean up the rule name for readability
                    short_name = rule_name.split(' :ruleset')[0][-50:]
                    print(f"  - Applied: ...{short_name} | Matches: {total_matches}")

        # 4. Extract result
        canonical_expr, cost = egraph.extract(e_root, include_cost=True)
...
print("\n" + "-"*50)
print("canonical_dsl from EGGLOG:")
print("-"*50)
